...
@pytest.mark.django_db
def test_approving_an_already_declined_signup_is_not_pending(client):
    shift = _shift(capacity=2)
    su = _signup(shift)
    hdr = _hdr(shift.shelter_account)
    client.post(f"/api/v1/shelter/signups/{su.pk}/decline", **hdr)
    res = client.post(f"/api/v1/shelter/signups/{su.pk}/approve", **hdr)
    assert res.status_code == 409
    assert res.json()["error"]["code"] == "not_pending"
    su.refresh_from_db()
    assert su.status == SignupStatus.DECLINED
    assert Notification.objects.filter(account=su.volunteer_account,
                                       type="shift_confirmed").count() == 0


@pytest.mark.django_db
def test_declining_an_already_approved_signup_is_not_pending(client):
    shift = _shift(capacity=2)
    su = _signup(shift)
    hdr = _hdr(shift.shelter_account)
    client.post(f"/api/v1/shelter/signups/{su.pk}/approve", **hdr)
    res = client.post(f"/api/v1/shelter/signups/{su.pk}/decline", **hdr)
    assert res.status_code == 409
    assert res.json()["error"]["code"] == "not_pending"
    su.refresh_from_db()
    assert su.status == SignupStatus.APPROVED
...
@pytest.mark.django_db(transaction=True)
def test_concurrent_approve_and_decline_on_the_same_signup_one_wins(client):
    """Approve and decline racing the SAME signup (double-tap, retry-on-timeout, two
    devices) must not oscillate the signup or produce a contradictory pair of
    notifications. Without the signup lock both sides can pass the pre-write status
    check, and whichever writes last silently overwrites the other's outcome."""
    for _ in range(3):
        shift = _shift(capacity=2)
        su = _signup(shift)
        hdr = _hdr(shift.shelter_account)
        ...
        def hit(action):
            try:
                barrier.wait(timeout=5)
                r = client.post(f"/api/v1/shelter/signups/{su.pk}/{action}", **hdr)
                results[action] = r.status_code
            finally:
                connection.close()  # each thread holds its own DB connection

        threads = [threading.Thread(target=hit, args=(a,)) for a in ("approve", "decline")]
        for t in threads:
            t.start()
        for t in threads:
            t.join(timeout=10)

        assert sorted(results.values()) == [200, 409], f"expected one win, got {results}"
        if results["approve"] == 409:
            assert results["decline"] == 200

        su.refresh_from_db()
        assert su.status in (SignupStatus.APPROVED, SignupStatus.DECLINED)

        confirmed = Notification.objects.filter(account=su.volunteer_account,
                                                 type="shift_confirmed").count()
        declined_notif = Notification.objects.filter(account=su.volunteer_account,
                                                      type="signup_declined").count()
        if su.status == SignupStatus.APPROVED:
            assert confirmed == 1 and declined_notif == 0, \
                f"approved but notifications were confirmed={confirmed} declined={declined_notif}"
        else:
            assert declined_notif == 1 and confirmed == 0, \
                f"declined but notifications were confirmed={confirmed} declined={declined_notif}"
